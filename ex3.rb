puts "I will count my chickens:"       #puts is used to print arguments in screen.
...
puts 3 + 2 < 5 - 7                          #3+2=5 , then 5-7=2. (i.e) 5<-2 is false. Here < is comparision operator.

puts "what is 3 + 2? #{3 + 2}"             #if we use operations inbetween {}. Here 3+2=5
...
puts "Is it greater? #{5 > -2}"               # 5 is greater than -2 , so result is true.
puts "Is it greater or equal? #{5 >= -2}"     #5 is greater than or equal to -2, so result is true.
puts "Is it less or equal? #{5 <= -2}"        # 5 is not less than or equal to -2, so results is false.


#2 I had started interactive ruby shell,also made some calculations.
#3 I had  completed using new file, which name is ex3new.rb
